...
test('Straight vs Straight (A-5 vs 2-6) [kicker]', () => {
  ...
  expect(hands[0].hand.name).toBe(HANDS_RANKING.STRAIGHT.name);
  expect(hands[0].hand.cards).toEqual(fistPlayerCards);

  expect(hands[1].player.name).toBe('Player 2');
  expect(hands[1].hand.name).toBe(HANDS_RANKING.STRAIGHT.name);
...
test('Straight (10-A)', () => {
  ...
  const boardCards = ['J♠', '5♣', 'K♠', 'Q♣', '4♠'].map(getCard);
